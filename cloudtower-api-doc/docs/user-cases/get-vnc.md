...
<>或者可以选择使用 `token`, `vm_uuid` 和 `host_ip`以构建通过 {Terminology['zh-CN']['PRODUCT']} 转发的 noVnc 链接，由于 token 是一个被 base64 处理后的字符串，因此中可能包含一部分不能包含在 URL 的字符，例如 `/`，`+`，`=` 等，需要转义成十六进制数字：</>
...
<>如果希望通过 {Terminology['zh-CN']['PRODUCT']} 转发，我们需要先处理 token，假设我们的 token 是 `MTIzNDU2Nzg5YWJjZGVmZ2hpamtsbW5vcHFyc3R1dnd4eXo=` （基于 `123456789abcdefghijklmnopqrstuvwxyz` base64 加密获得），我们需要将 `/`，`+`，`=` 等字符转义。</>
