...
<>{Terminology['en-US']['STORAGE_PRODUCT']}</> volume 指最终的存储对象，使用 ZADP 可以与 <>{Terminology['en-US']['STORAGE_PRODUCT']}</> volume 交互，完成数据的备份和恢复。
### 获取所需虚拟机的基本信息。
...
每一个 LUN 快照的 local_id 字段即在 <>{Terminology['en-US']['STORAGE_PRODUCT']}</> 中的标识符，可以通过 <>{Terminology['en-US']['STORAGE_PRODUCT']}</> API 进行进一步交互。