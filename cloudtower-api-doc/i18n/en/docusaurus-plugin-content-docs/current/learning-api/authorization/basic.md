...
<>From the quick start example, you can see that using the {Terminology['en-US']['PRODUCT']} API consists of 2 steps: first, obtain an authentication token by logging in; second, carry the token in the Authorization header and call the corresponding API to perform operations.</>
