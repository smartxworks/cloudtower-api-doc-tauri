...
<>The return value of {Terminology['terminology']['en-US']['PRODUCT']} API requests that perform changes is either a <code>WithTask{`<T>`}</code> object or <code>Array{`<WithTask<T>>`}</code> objects. <code>WithTask</code> is a generic type that contains <code>task_id</code> and the corresponding return value. </>
